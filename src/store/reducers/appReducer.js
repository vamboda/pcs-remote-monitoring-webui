...
const handleError = fromAction => error =>
  Observable.of(redux.actions.registerError(fromAction.type, {
    error,
    fromAction
  }));
...
const updateDeviceGroupsReducer = (state, {
  payload,
  fromAction
}) => {
  const {
...
const deleteDeviceGroupsReducer = (state, {
  payload
}) => update(state, {
  deviceGroups: {
    $unset: [...payload]
...
const insertDeviceGroupsReducer = (state, {
  payload
}) => {
  const {
    entities: {
...
const updateAzureMapsKeyReducer = (state, {
  payload,
  fromAction
}) => update(state, {
  azureMapsKey: {
...
const updateThemeReducer = (state, {
  payload
}) => update(state, {
  theme: {
    $set: payload
...
const updateTimeInterval = (state, {
  payload
}) => update(state, {
  timeInterval: {
    $set: payload
...
const logoReducer = (state, {
  ...
  logo: {
    $set: payload.logo ? payload.logo : svgs.contoso
  },
  name: {
    $set: payload.name ? payload.name : 'companyName'
  },
  isDefaultLogo: {
    $set: payload.logo ? false : true
  },
  ...setPending(fromAction.type, false)
});

const releaseReducer = (state, {
  payload
}) => update(state, {
  version: {
    $set: payload.version
  ...
  releaseNotesUrl: {
    $set: payload.releaseNotesUrl
  }
});

const setDeviceGroupFlyoutReducer = (state, {
  payload
}) => update(state, {
...
export const redux = createReducerScenario({
  ...
  updateAzureMapsKeyGroup: {
    type: 'APP_AZURE_MAPS_KEY_UPDATE',
    reducer: updateAzureMapsKeyReducer
  },
  ...
  registerError: {
    type: 'APP_REDUCER_ERROR',
    reducer: errorReducer
  },
  isFetching: {
    multiType: fetchableTypes,
...
export const reducer = {
  app: redux.getReducer(initialState)
};
// ========================= Reducers - END
